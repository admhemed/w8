...
export class PbiService {
    ...
    public auth() {
        const authenticationContext = new this.winRef.nativeWindow.AuthenticationContext({
                instance: 'https://login.microsoftonline.com/',
                clientId: '3867b3a9-393e-4b9c-b8af-421d61f1ffde',
        ...
        if (user) {
            if (!renewActive) {
                authenticationContext.acquireToken(resource, (errorDesc, t, error) => {
                    if (!error) {
                        // yourLogic(token);
                    } else {
